TabPanel = function (a, b) {
    this.i18n = {closeTab: "关闭页签", refreshTab: "刷新页签", closeOther: "关闭其他", closeLeft: "关闭左侧", closeRight: "关闭右侧",};
    if (window.lang == "en") {
        this.i18n = {
            closeTab: "Close",
            refreshTab: "Refresh",
...
TabPanel.prototype = {
    init: function () {
        ...
        if ($("#" + c.id).length > 0) {
            ...
            if (this.scrollFinish) {
                ...
                if (c.icon) {
                    ...
                    if (j.width() > (this.tabWidth - 35 - g)) {
                        j.width((this.tabWidth - 57 - g));
                        j.attr("title", c.title);
                        d.append("<DIV>...</DIV>")
                    }
                } else {
                    j.addClass("title");
                    if (j.width() > (this.tabWidth - 19 - g)) {
                        j.width((this.tabWidth - 37 - g));
                        j.attr("title", c.title);
                        d.append("<DIV>...</DIV>")
                    }
                }
        ...
        for (var d = 0; d < this.tabs.length; d++) {
            this.tabs[d].tab.unbind("click");
            this.tabs[d].tab.bind("click", function (f) {
                return function () {
                    e.show(f, false)
                }
            }(d));
            this.tabs[d].refresher.unbind("click");
            this.tabs[d].refresher.bind("click", function (f) {
                return function () {
                    e.refresh(f)
                }
            }(d));
            this.tabs[d].closer.unbind("click");
            this.tabs[d].closer.bind("click", function (f) {
                return function () {
                    e.kill(f)
                }
            }(d));
            if (this.tabs[d].closable) {
                this.tabs[d].tab.unbind("dblclick");
                this.tabs[d].tab.bind("dblclick", function (f) {
                    return function () {
                        e.kill(f)
                    }
                }(d))
